// // ──────────────────────────────────────────────────────────────
// //  varint_g8cu.cpp  –  2‑bit × 4 값 G8CU (+ AVX2 decode fast‑path)
// // ──────────────────────────────────────────────────────────────
...
// constexpr int GROUP = 4;                 // 4 values = 1 descriptor
...
//     while (p < in + n) {
//         /* FAST‑PATH : 4 one‑byte values → sentinel 0xFF + raw bytes */
//         if (p + GROUP <= in + n &&
...
//         /* General G8CU coding (2‑bit length × 4 values) */
//         uint8_t *desc = o++;
//         uint8_t bits = 0;
//         for (int i = 0; i < GROUP; ++i) {
//             uint32_t v = (p < in + n) ? *p++ : 0;
//             uint8_t lc = len_code(v);          // 0–3
//             bits |= lc << (i * 2);             // put in {0,2,4,6} bits
//             for (int b = 0; b <= lc; ++b) {
//                 *o++ = uint8_t(v);
//                 v >>= 8;
//             }
...
// /*───────────────────── internal: AVX2 fast copy ───────────────────*/
// #ifdef __AVX2__
// static inline void decode_ff_fast(const uint8_t *&p, uint32_t *&o) {
//     /* p points to 4 raw one‑byte values (after sentinel).               */
//     /* Load 4 bytes → zero‑extend to 4×uint32 with one AVX2/SSE op.      */
//     uint32_t packed = *reinterpret_cast<const uint32_t *>(p);
//     __m128i v4 = _mm_cvtepu8_epi32(_mm_cvtsi32_si128(packed)); // 4 → 4×32‑bit
//     _mm_storeu_si128(reinterpret_cast<__m128i *>(o), v4);
//     p += 4;
//     o += GROUP;
// }
// #else
...
//     while (o < end) {
//         /* FAST‑PATH : sentinel 0xFF → next 4 bytes are literal values */
//         if (*p == 0xFF) {
//             ++p;                  // consume sentinel
//             decode_ff_fast(p, o); // AVX2‑accelerated copy
//             continue;
//         }
...
// ──────────────────────────────────────────────────────────────
//  varint_g8cu.cpp  –  2‑bit × 4 값 G8CU (+ **SSE4.1** decode fast‑path)
//    * Target: 동일 ISA(SSE4.1) 조건에서 StreamVByte와 공정 비교용 *
// ──────────────────────────────────────────────────────────────
#include "varint_g8cu.h"
